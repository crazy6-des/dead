...
assert.ok(PRIMARY_NAVIGATION.every((item) => typeof item.icon === "function"));

assert.deepEqual(
  HEADER_NAVIGATION.map((item) => item.route),
  [APP_ROUTES.NOTIFICATIONS, APP_ROUTES.PROFILE],
);
assert.ok(HEADER_NAVIGATION.every((item) => typeof item.icon === "function"));
...
assert.ok(MOBILE_NAVIGATION.filter((item) => item.route).every((item) => typeof item.icon === "function"));

assert.deepEqual(
  SECONDARY_NAVIGATION.map((item) => item.route),
  [APP_ROUTES.SPACES, APP_ROUTES.SAVED, APP_ROUTES.LISTS, APP_ROUTES.SETTINGS],
);
assert.ok(SECONDARY_NAVIGATION.every((item) => typeof item.icon === "function"));
