...
export default function HomeRoute({ posts, onLike, onSave, onFollow, onCreate, onOpen }) {
  ...
  return <><div className="feed-tabs">{["For You", "Following", "Latest"].map((name) => <button key={name} className={tab === name ? "selected" : ""} onClick={() => setTab(name)}>{name}</button>)}</div>
    ...
    {visible.map((post) => <PostCard key={post.id} post={post} onLike={onLike} onSave={onSave} onFollow={onFollow} onOpen={onOpen}/>)}
  </>;
}
