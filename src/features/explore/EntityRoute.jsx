...
function UserDetail({ username, onBack, onOpen, onLike, onSave, onRepost, onFollowUser, followingUsers = new Set() }) {
  ...
  useEffect(() => { let active = true; setActivityLoading(true); setActivityError(""); profileService.listPosts(user, tab).then((result) => { if (active) setActivity(Array.isArray(result?.items) ? result.items : []); }).catch((cause) => { if (active) setActivityError(cause?.message || "Profile activity could not be loaded."); }).finally(() => { if (active) setActivityLoading(false); }); return () => { active = false; }; }, [user, tab]);
  ...
  </div></div><div className="entity-tabs">{["posts", "replies", "media", "likes"].map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item.charAt(0).toUpperCase() + item.slice(1)}</button>)}</div>
