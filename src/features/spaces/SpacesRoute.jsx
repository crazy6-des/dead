...
const LiveAudio = forwardRef(function LiveAudio({ space, userRole, onRoleChange, onSelfRoleChange, onChat, onPresence }, ref) {
  const selfUserIdRef = useRef(null), wsRef = useRef(null), peersRef = useRef(new Map()), streamRef = useRef(null), audioRef = useRef(new Map());
  ...
  useImperativeHandle(ref, () => ({
    sendChat(text) { if (wsRef.current?.readyState !== WebSocket.OPEN) return false; wsRef.current.send(JSON.stringify({type:"chat",text})); return true; },
    sendRole(targetUserId, role) { if (wsRef.current?.readyState !== WebSocket.OPEN) return false; wsRef.current.send(JSON.stringify({type:"role",targetUserId,role})); return true; },
  }), []);
  useEffect(() => {
    ...
    ws.onmessage=async(event)=>{
      ...
      if(msg.type==="ready"){ selfUserIdRef.current=msg.userId; for(const peer of msg.peers||[]) await createPeer(peer.userId,true); return; }
      if(msg.type==="signal"){
...
function SpaceRoom({ space, onBack, onRefresh }) {
  ...
  return <div className="space-room">
    ...
    <div className="space-room__grid">
      <section className="space-stage"><div className="space-host"><Avatar name={detail.host} src={detail.hostAvatarUrl}/><div><strong>{detail.host}</strong><span>@{detail.hostUsername} · Host</span></div></div><h3>{detail.title}</h3><p>Live audio on S. Speak, listen, and participate with the people in this Space.</p><LiveAudio ref={audioRef} space={detail} userRole={detail.role} onRoleChange={roleChange} onSelfRoleChange={(role)=>setDetail((current)=>({...current,role,canSpeak:role==="host"||role==="speaker"}))} onPresence={load}
      onChat={(message)=>setMessages((all)=>all.some((item)=>item.id===message.id)?all:[...all,{...message,text:message.text,sender:message.sender}])}/>{detail.role==="host"&&<div className="space-owner-actions"><button className="danger-outline" onClick={end}>End Space</button><button className="danger-outline space-delete" onClick={remove}>Delete Space</button></div>}{detail.role!=="host"&&<button className="outline" onClick={leave}>Leave Space</button>}</section>
      <aside className="space-room__side"><div className="space-panel"><h3><Users size={15}/> Participants · {members.length}</h3>{members.map((m)=><div className="space-member" key={m.id}><Avatar name={m.name} src={m.avatarUrl}/><div><b>{m.name}</b><span>@{m.username} · {m.role}</span></div>{detail.role==="host"&&m.role!=="host"&&<button className="outline" onClick={async()=>{const role=m.role==="speaker"?"listener":"speaker";try{await spaceService.setRole(detail.id,m.id,role);audioRef.current?.sendRole(m.id,role);setMembers((all)=>all.map((x)=>x.id===m.id?{...x,role}:x));}catch(err){setError(err?.message||"Could not change role.");}}}>{m.role==="speaker"?"Make listener":"Invite to speak"}</button>}</div>)}</div>
