...
const descriptions = {
  ...
  Privacy: "Control account visibility and follower-count display.",
...
export default function SettingsRoute({ onSettingsUpdate, onOpen }) {
  ...
  return <div className="page">
      ...
      {items.map((item) => {
        ...
        return <div className="setting-row" key={item}>
          ...
          {expanded && <div id={sectionId} className="setting-panel">
            ...
            {item === "Privacy" && <>
              <label className="setting-toggle"><input type="checkbox" checked={settings.privateAccount} onChange={(event) => update({ privateAccount: event.target.checked })} /><span><b>Private account</b><small>Only approved followers can view your posts.</small></span></label>
              <label className="setting-toggle"><input type="checkbox" checked={settings.showFollowerCount} onChange={(event) => update({ showFollowerCount: event.target.checked })} /><span><b>Show follower count</b><small>Display your follower total on your profile.</small></span></label>
              <label className="setting-toggle"><input type="checkbox" checked={settings.discoverable} onChange={(event) => update({ discoverable: event.target.checked })} /><span><b>Discoverable profile</b><small>Allow your profile to appear in discovery.</small></span></label>
            </>}
            ...
            {item === "Notifications" && <label className="setting-toggle"><input type="checkbox" checked={settings.allowMessages} onChange={(event) => update({ allowMessages: event.target.checked })} /><span><b>Allow messages</b><small>Control whether people can start conversations with you.</small></span></label>}
    ...
    <section className="settings"><h3>Privacy status</h3><div className="setting-callout"><Lock /><span><b>{settings.privateAccount ? "Private account" : "Public account"}</b><small>{settings.privateAccount ? "Your posts are intended for approved followers." : "Your profile is publicly discoverable according to your account settings."}</small></span></div></section>
  </div>;
}
