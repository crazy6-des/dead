...
function validateAsset(asset, { kind, index, maxSize, allowedTypes }) {
  ...
  if (Number.isFinite(asset.size) && asset.size > maxSize) {
    errors.push(kind === "image" ? `Image ${index + 1} exceeds the 10 MB limit.` : "Audio exceeds the 20 MB limit.");
  }
  return errors;
}
...
function validateAudioAsset(asset) {
  ...
  return validateAsset(asset, { kind: "audio", index: 0, maxSize: MAX_AUDIO_SIZE, allowedTypes: AUDIO_TYPES });
}
...
export function validatePostDraft(draft) {
  ...
  if (!Array.isArray(payload.media)) {
    ...
    const mediaErrors = payload.media.flatMap((asset, index) => validateAsset(asset, { kind: "image", index, maxSize: MAX_IMAGE_SIZE, allowedTypes: IMAGE_TYPES }));
