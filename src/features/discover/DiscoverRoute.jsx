...
export default function DiscoverRoute({ posts, onOpen, onFollow }) {
  ...
  return <div className="page">
    ...
    {(tab === "For you" || tab === "People") && <section className="card"><header><div><small>PEOPLE</small><h2>People to connect</h2></div><Users size={18}/></header>{people.map((name) => <div className="person" key={name}><button className="avatar avatar--small" onClick={() => onOpen?.("/user/" + name.split(" ")[0].toLowerCase())}>{name[0]}</button><div><strong>{name}</strong><span>@{name.split(" ")[0].toLowerCase()} · Creator</span></div><button className="follow" onClick={() => onFollow?.(name)}>Follow</button></div>)}</section>}
    {(tab === "For you" || tab === "Posts") && <section className="card discover-posts"><header><div><small>POSTS</small><h2>Posts for you</h2></div></header>{filtered.slice(0,8).map((p) => <PostCard key={p.id} post={p} onLike={() => {}} onSave={() => {}} onFollow={() => {}} onOpen={onOpen}/>)}</section>}
