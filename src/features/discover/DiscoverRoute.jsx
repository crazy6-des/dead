...
export default function DiscoverRoute({ posts, onLike, onSave, onOpen, onFollow, followingUsers = new Set() }) {
  ...
  return <div className="page">
    ...
    {(tab === "For you" || tab === "Posts") && <section className="card discover-posts"><header><div><small>POSTS</small><h2>Posts for you</h2></div></header>{filtered.slice(0,8).map((p) => <PostCard key={p.id} post={p} onLike={onLike} onSave={onSave} onFollow={() => onFollow?.(String(p.h || "").replace("@", "").toLowerCase())} onOpen={onOpen}/>)}</section>}
