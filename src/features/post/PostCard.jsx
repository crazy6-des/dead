...
export default function PostCard({ post, onLike, onSave, onFollow, onRepost, onOpen }) {
  ...
  return <article className="post" onDoubleClick={() => onLike?.(post.id)}>
    ...
    <div className="post__body">
      ...
      {post.poll && <div className="poll-card"><div className="poll-card__question"><BarChart3 size={17}/><strong>{post.poll.question}</strong></div>{(post.poll.options || []).map((option, index) => { const selected = pollVotes[post.poll.id || post.id] === index; const total = Number(post.poll.totalVotes || 0); const votes = Number(option.votes || 0); const percent = total > 0 ? Math.round((votes / total) * 100) : 0; return <button className={"poll-option " + (selected ? "is-selected" : "")} key={index} onClick={async () => { if (pollBusy) return; const pollId = post.poll.id || post.id; setPollBusy(true); setPollError(""); try { await pollService.vote(pollId, index); setPollVotes((current) => ({ ...current, [pollId]: index })); } catch { setPollError("Could not record your vote."); } finally { setPollBusy(false); } }}><span>{option.text || option}</span><span>{percent}%</span></button>; })}<small>{pollError || `${post.poll.totalVotes || 0} votes`}</small></div>}
