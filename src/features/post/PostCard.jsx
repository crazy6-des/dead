...
export default function PostCard({ post, onLike, onSave, onFollow, onRepost, onOpen }) {
  ...
  return <article className="post" onDoubleClick={() => onLike?.(post.id)}>
    ...
    <div className="post__body">
      ...
      {mediaSources.length > 0 && <div className="post-media-grid">{mediaSources.map((source, index) => <button className="post-media" key={source + index} onClick={() => onOpen?.("/post/" + encodeURIComponent(post.id))}><img src={source} alt={imageItems[index]?.alt || "Post media"} loading="lazy" /></button>)}</div>}
