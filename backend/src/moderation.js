...
async function sendReportEmail(env,report){
  ...
  const html=`<div style="font-family:Arial,sans-serif;line-height:1.6"><h2>New S moderation report</h2><table cellpadding="6"><tr><td><b>Report ID</b></td><td>${escape(report.id)}</td></tr><tr><td><b>Reporter</b></td><td>@${escape(report.reporterUsername)}</td></tr><tr><td><b>Target</b></td><td>${escape(report.targetType)} / ${escape(report.targetId)}</td></tr><tr><td><b>Reason</b></td><td>${escape(report.reason)}</td></tr><tr><td><b>Note</b></td><td>${escape(report.note||"")}</td></tr><tr><td><b>Submitted</b></td><td>${escape(report.createdAt)}</td></tr><tr><td><b>URL</b></td><td>${escape(report.url||"")}</td></tr></table></div>`;
...
export async function moderationAction(request,env){
  ...
  let targetRow=null;
  if(targetType==="post")targetRow=await env.DB.prepare("SELECT p.id,p.author_id,u.username FROM posts p JOIN users u ON u.id=p.author_id WHERE p.id=?1 AND p.deleted_at IS NULL LIMIT 1").bind(targetId).first();
  else if(targetType==="message")targetRow=await env.DB.prepare("SELECT m.id,m.sender_id,u.username FROM messages m JOIN users u ON u.id=m.sender_id WHERE m.id=?1 AND m.deleted_at IS NULL LIMIT 1").bind(targetId).first();
  ...
  try{emailStatus=await sendReportEmail(env,{id,reporterUsername:reporter?.username||session.user_id,targetType,targetId,reason,note,createdAt,url:request.url})?"sent":"not_configured";}catch(error){console.error("MODERATION_REPORT_EMAIL_FAILED",error);emailStatus="failed";}
