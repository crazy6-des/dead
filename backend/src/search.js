...
export const DISCOVER_NICHE_KEYWORDS = Object.freeze({
...
export async function search(request, env) {
  ...
  const nicheTerms = DISCOVER_NICHE_KEYWORDS[niche] || [];
  if (!query && !nicheTerms.length) {
  ...
  if (type === "all" || type === "posts") {
    ...
    if (query) {
      clauses.push("p.body LIKE ? ESCAPE '\\'");
      values.push(like);
    }
    values.push(limit);
    postRows = await env.DB.prepare(
      `SELECT p.id,p.author_id,p.body,p.visibility,p.reply_policy,p.post_kind,p.background_json,p.quoted_post_id,p.created_at,p.updated_at,u.username,u.display_name,(p.author_id=?) AS is_owner,
  ...
  return {
    response: {
      ...
      query,
      type,
      niche: nicheTerms.length ? niche : null,
    },
    error: null,
  };
}
