...
const state = {
  pollJson: JSON.stringify({ question: "Which one?", options: ["Alpha", "Beta"], multipleChoice: false, totalVotes: 0, optionVotes: [0, 0], votedOptionIndex: null }),
  votes: [],
};
      ...
      bind(...values) {
        return {
          async first() {
            ...
            if (query.startsWith("SELECT id, author_id, deleted_at, poll_json FROM posts")) {
              return { id: "poll-1", author_id: "author-1", deleted_at: null, poll_json: state.pollJson };
            }
            if (query.startsWith("SELECT option_index FROM poll_votes")) {
              const vote = state.votes.find((item) => item.user_id === values[1]);
              return vote ? { option_index: vote.option_index } : null;
            }
            return null;
          },
          async all() {
            if (query.includes("SELECT option_index, COUNT(*) AS count FROM poll_votes")) {
              ...
              for (const vote of state.votes) grouped.set(vote.option_index, (grouped.get(vote.option_index) || 0) + 1);
          ...
          async run() {
            if (query.startsWith("INSERT OR IGNORE INTO poll_votes")) {
              if (state.votes.some((vote) => vote.user_id === values[1])) return { meta: { changes: 0 } };
              state.votes.push({ user_id: values[1], option_index: Number(values[2]) });
              return { meta: { changes: 1 } };
            }
            if (query.startsWith("UPDATE posts SET poll_json")) state.pollJson = values[0];
            return { meta: { changes: 1 } };
          },
        };
      },
...
const request = (optionIndex) => new Request("https://example.test/api/polls/poll-1/votes", {
  method: "POST",
  headers: { Cookie: "s_session=test-session", "content-type": "application/json" },
