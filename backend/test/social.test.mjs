...
const state = { posts: [{ id: "post-1", author_id: "user-2", deleted_at: null }], reactions: new Set(), bookmarks: new Set() };
const db = { prepare(query) { return { bind(...values) { return {
  async first() {
    if (query.startsWith("SELECT s.id")) return values[0] === await sha256Hex("session-1") ? { id: "session-1", user_id: "user-1", username: "user1", display_name: "User 1" } : null;
    ...
    if (query.startsWith("SELECT COUNT(*)")) return { count: query.includes("FROM bookmarks") ? [...state.bookmarks].filter((key) => key.endsWith(`:${values[0]}`)).length : [...state.reactions].filter((key) => key.endsWith(`:${values[0]}:${values[1]}`)).length };
    return null;
  },
  async run() {
...
const request = (path, method = "POST", body) => new Request(`https://example.test${path}`, { method, headers: { Cookie: "s_session=session-1", ...(body === undefined ? {} : { "content-type": "application/json" }) }, ...(body === undefined ? {} : { body: JSON.stringify(body) }) });
